...
int[,] serialMulRes = new int[N, N]; //результат выполнения умножения матриц в однопотоке (последовательное умножения матриц)
...
SerialMatrixMult(firstMatrix, secondMatrix); //последовательное умножение
PrepareParallelMatrixMul(firstMatrix, secondMatrix); //параллельное умножение
Console.WriteLine(EqualityMatrix(serialMulRes, threadMulRes)); //true - если результаты умножения совпадают
...
void SerialMatrixMult(int[,] a, int[,] b)
{
...
void PrepareParallelMatrixMul(int[,] a, int[,] b)
{
    ...
    for (int i = 0; i < THREADS_NUMBER; i++)
    {
        ...
        threadsList.Add(new Thread(() => ParallelMatrixMult(a, b, startPos, endPos)));
        threadsList[i].Start();
    }
    for (int i = 0; i < THREADS_NUMBER; i++)
    {
        threadsList[i].Join(); //склеивает отдельный поток с главным потоком
    }
}

void ParallelMatrixMult(int[,] a, int[,] b, int startPos, int endPos)
{
